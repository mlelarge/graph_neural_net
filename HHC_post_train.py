...
if __name__=='__main__':
    ...
    mu_to_test = [fill_param_list[number] for number in l_model_used]
    ...
    for mu in pb:
        ...
        if not os.path.isfile(filepath):
            with open(filepath,'w') as f:
                f.write('fill_param_test,acc,perf_hhc,auc_tsps,auc_tsp,tsp_length,tsps_length,inf_length,tsps_tsp_len_ratio,tsps_tsp_edge_ratio,inf_tsp_len_ratio,inf_tsp_edge_ratio\n')
        else:
        ...
        pb.set_description(desc=f'Using mu={mu}')
        gen_args['fill_param'] = mu

        if check_model_exists(model_path,n_vertices,mu): #If model already exists
            print(f'Using already trained model for mu={mu}')
            model = get_model(model_args)
            state_dict = load_model_dict(model_path,n_vertices,mu,device)
            ...
            save_model(model_path, model, n_vertices, mu)
        
        counter=0
        for fill_param in tqdm.tqdm(fill_param_list):
            if counter<n_lines:
                print(f'Skipping train_mu={mu}, test_mu={fill_param}')
            else:
