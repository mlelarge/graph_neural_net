...
torch.save(testset_dense, '/home/mlelarge/data/superpixels/MNIST/mnist_test.pt')

valset = [dataset.collate_dense_gnn([d]) for d in dataset.val]
valset = [(g.squeeze(0), l) for (g,l) in valset]
torch.save(valset, '/home/mlelarge/data/superpixels/MNIST/mnist_val.pt')

trainset = [dataset.collate_dense_gnn([d]) for d in dataset.train]
trainset = [(g.squeeze(0), l) for (g,l) in trainset]
torch.save(trainset, '/home/mlelarge/data/superpixels/MNIST/mnist_train.pt')